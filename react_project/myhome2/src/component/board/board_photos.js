...
function PhotoList() {
  ...
  const hosturl = 'https://jsonplaceholder.typicode.com/photos';
    ...
    <div>
        ...
        <table className="table table-hover ">
          <colgroup>
            ...
            {
              loading==true?
              PhotoList.map((TodoItem, key)=>{
                return(
                  <tr key={key}>
                    <td>{TodoItem.albumId}</td>
                    <td style={{"textAlign":"right"}}>{TodoItem.id}</td>
                    <td>{TodoItem.title}</td>
                    <td><a href={TodoItem.url}>{TodoItem.url}</a></td>
                    <td><img src={TodoItem.thumbnailUrl}></img></td>
                  </tr>)
